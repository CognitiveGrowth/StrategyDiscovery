classdef MouselabExperiment2 < MDP
    ...
    properties
        ...
        time_per_click=1; %assumption: about 40sec per trial and 8 out of the 16 boxes are opened
        time_cost_per_sec=7/3600; %7$/h in points/sec
        ...
        action_features;
        payoff_ranges=[0.01 9.99; 0.00 0.25];
        nr_trials=20;
    end
    
    methods
        function mdp=MouselabExperiment2()
        ...
        function [state,mdp]=newEpisode(mdp)
            ...
            mdp.payoff_values=linspace(mdp.min_payoff,mdp.max_payoff,10);
        ...
        function outcome_probabilities=sampleOutcomeProbabilities(mdp)
            ...
            if mdp.high_dispersion
                ...
                while or(max(outcome_probabilities)<0.85,max(outcome_probabilities)>0.97)
                    temp=rand(mdp.nr_outcomes,1);
                    outcome_probabilities=temp/sum(temp);
                end
                ...
                while or(max(outcome_probabilities)>0.4,min(outcome_probabilities)<0.1)
                    temp=rand(mdp.nr_outcomes,1);
                    outcome_probabilities=temp/sum(temp);
                end
        ...
        function action_features=extractActionFeatures(mdp,state,action)
            ...
            if action.is_decision
                ...
                observation=1;
                decision=0;
                regret_reduction=mdp.regretReductionMinusCost(state,action);
                %early_decision=0;
            ...
            action_features=[%ER
                ...
                %early_decision;                
               observation; regret_reduction%;decision
                ];
            %}
            
            %action_features=mdp.myopicVOC(state,action);
            %action_features=mdp.regretReductionMinusCost(state,action);
            %action_features=mdp.twoStepVOC(state,action);
        end
...
%             disp(['mu: ',num2str(state.mu(1)),' sigma: ',num2str(state.sigma(1)),' len: ',num2str(length(state.mu))])
        ...
        function VOC=twoStepVOC(mdp,state,action)
            if and(state.time_left>mdp.time_per_click,not(action.is_decision))
                ...
                if isnan(state.observations(action.outcome,action.gamble))
                    ...
                    for s=1:numel(next_states)
                        next_actions=mdp.getActions(next_states(s));
                        next_step_VOC=NaN(numel(next_actions),1);
                        for a=1:numel(next_actions)
                            next_step_VOC(a)=mdp.myopicVOC(next_states(s),next_actions(a));
                        end
                        delta_EV(s)=max(next_step_VOC);
                    end
                    
                    VOC=dot(p_next_states,delta_EV)-mdp.time_cost_per_sec*mdp.time_per_click;
                else
                    VOC=0;
                end
        ...
        function regret_reduction_minus_cost=regretReductionMinusCost(mdp,s_old,action)

            
            [next_states,p_next_states]=mdp.predictNextState(s_old,action);
            
            for s=1:numel(next_states)
                regret_reduction(s)=regretReduction(mdp,s_old,next_states(s));
            end
            E_regret_reduction=dot(p_next_states,regret_reduction);
            
            regret_reduction_minus_cost=E_regret_reduction-mdp.time_per_click*mdp.time_cost_per_sec;
        end
        ...
        function regret_reduction=regretReduction(mdp,s_old,s_new)
            ...
            regret_reduction=max(0,Phi_new-Phi_old); %TODO: check whether regret reduction is always non-negative
        end
        ...
        function mdp=setPayoffRange(mdp,min_payoff,max_payoff)
            ...
            mdp.payoff_values=linspace(mdp.min_payoff,mdp.max_payoff,15);
            payoff_range=mdp.max_payoff-mdp.min_payoff;
            mdp.std_payoff=payoff_range*0.3;%2/1.96;
            mdp.p_payoff_values=discreteNormalPMF(mdp.payoff_values,...
                mdp.mean_payoff,mdp.std_payoff);
            mdp.nr_possible_payoffs=numel(mdp.payoff_values);
        end

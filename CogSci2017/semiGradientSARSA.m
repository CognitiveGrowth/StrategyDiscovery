function [w,avg_MSE,R_total]=semiGradientSARSA(mdp,feature_extractor,nr_episodes,epsilon)
...
for i=1:nr_episodes    
    ...
    while not(mdp.isTerminalState(s))
        t=t+1;
        %1. Choose action
        epsilon=1/sqrt(nr_observations+2);
        ...
        for a=1:numel(actions)
            ...
            next_state_values=feature_extractor(next_states,mdp)'*w;
        ...
        if mdp.isTerminalState(s_next)
            ...
            value_estimate=r+PR+mdp.gamma*dot(feature_extractor(s_next,mdp),w);
        end
